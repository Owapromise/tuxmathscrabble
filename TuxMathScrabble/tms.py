...
class TuxMathScrabble:
	...
	def __init__(self,mode,level,use_default_level,USE_WX):
		...
		if USE_WX:
			import TuxMathScrabble.wxadmin
	...
	def make_tile(self,strval,uchar,ptval):
		...
		if self.global_config['TILES_TWO_COLORS']['value']==1 and self.player_idx==1:
			...
			ptval_surface=self.pfont.render(str(tile.ptval),1,self.global_config['COLOR_FG_PLAYER1_TILE']['value'],self.global_config['COLOR_BG_PLAYER1_TILE']['value'])
			...
			ptval_surface=self.pfont.render(str(tile.ptval),1,self.global_config['COLOR_FG_PLAYER0_TILE']['value'],self.global_config['COLOR_BG_PLAYER0_TILE']['value'])
	...
	def show_credit(self):
		...
		msgs=[
			...
			u'',
			u'TuxMathScrabble Version 0.9.0',
			u'June 10, 2020',
			u'',
			u'Author:Charlie Coss'+u'\xe9',
			u'Contact:ccosse@gmail.com',
			u'Website: ccosse.github.io',
		]
	...
	def on_exit(self):
		lines=[
			...
			'**********************************************************',
			'*                                                        *',
			'*     You are using version 0.9.0 from June 10, 2020     *',
			'*                                                        *',
